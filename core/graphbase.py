...
from src.models.embedding import import_flag_embedding
...
class GraphDatabase:
    ...
    def start(self):
        ...
        try:
            self._driver = Neo4jDriver.driver(f"{uri}/{self.kgdb_name}", auth=(username, password))
            self.status = "open"
    ...
    def txt_add_vector_entity(self, triples, kgdb_name='neo4j'):
        ...
        with self._driver.session() as session:
            session.execute_write(self._create_graph, triples)
        
        # 创建向量索引
        from src.config import EMBED_MODEL_INFO
        embed_info = EMBED_MODEL_INFO[self.config.embed_model]
        with self._driver.session() as session:
            session.execute_write(self._create_vector_index, embed_info.get('dimension'))
    ...
    def _create_vector_index(self, tx, dim):
        """创建向量索引的内部方法"""
        index_name = "entityEmbeddings"
        if not self._index_exists(tx, index_name):
            tx.run(f"""
    ...
    def _index_exists(self, tx, index_name):
        """检查索引是否存在的内部方法"""
    ...
    def get_embedding(self, text):
        ...
        try:
            ...
            return embeddings[0].tolist() if len(text) == 1 else [e.tolist() for e in embeddings]
        except Exception as e:
    ...
    def set_embedding(self, tx, entity_name, embedding):
        ...
        try:
            ...
            """
            tx.run(query, entity_name=entity_name, embedding=embedding)
        except Exception as e:
            logger.error(f"设置实体向量失败: {str(e)}")
            raise
