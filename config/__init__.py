...
with open(Path("F:/知识库/jbkbnew/src/config/models.yaml"), "r",encoding='utf-8') as f:
...
class Config(SimpleConfig):
    ...
    def handle_self(self):
        ...
        logger.info(f"=== 配置处理调试信息 ===")
        logger.info(f"当前 model_provider: {self.model_provider}")
        logger.info(f"当前 model_name: {self.model_name}")
        logger.info(f"模型提供商信息: {model_provider_info}")
        ...
        if self.model_provider != "custom":
            ...
            logger.info(f"最终使用的 model_name: {self.model_name}")
        else:
        ...
        for provider in self.model_names:
            ...
            logger.info(f"提供商 {provider} 状态: {self.model_provider_status[provider]} (环境变量: {conds[provider]})")

        self.valuable_model_provider = [k for k, v in self.model_provider_status.items() if v]
        logger.info(f"可用的模型提供商: {self.valuable_model_provider}")
