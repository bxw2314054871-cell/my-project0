...
async def create_database(
    ...
    """创建新的数据库"""
    logger.info(f"Creating database: name={database_name}, type={db_type}, dimension={dimension}")
...
async def create_document_by_file(
    ...
    对于Excel文件，可以通过params参数传递以下配置：
    {
        "excel_params": {
            "sheet_name": 0,  # 工作表索引或名称，默认为第一个工作表
            "header": 0,      # 标题行索引，默认为第一行
            "encoding": "utf-8"
        }
    }
    """
    logger.debug(f"Add document in {db_id} by file: {files}")
    msg = startup.dbm.add_files(db_id, files, params=params)
    return msg


@data.post("/add-by-file-auto")
async def create_document_by_file(
        db_id: str = Body(...),
        files: List[str] = Body(...),
        params: Optional[dict] = Body(None)
    ...
    """
    logger.debug(f"Add document in {db_id} by file: {files}")
    msg = startup.dbm.add_filesAuto(db_id, files, params=params)
    return msg
@data.get("/info")
...
async def upload_file(file: UploadFile = File(...)):
    ...
    try:
       ...
       #upload_dir = "F:/data/uploads"
        ...
        # 移除特殊字符，保留中文、英文、数字
        import re
        safe_basename = re.sub(r'[^\w\u4e00-\u9fa5]', '_', basename)

        hash_value = hashstr(basename, 4, with_salt=True)
        safe_filename = f"{safe_basename}_{hash_value}{ext}".lower()
